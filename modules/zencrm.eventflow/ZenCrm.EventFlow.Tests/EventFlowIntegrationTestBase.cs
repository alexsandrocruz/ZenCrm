...
namespace ZenCrm.EventFlow.Tests;

/* Inherit your integration test classes from this class */
public abstract class EventFlowIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(EventFlowModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(EventFlowModule).Assembly.FullName} assembly!");
        }
