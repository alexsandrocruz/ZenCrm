...
namespace ZenCrm.Finance.Tests;

/* Inherit your integration test classes from this class */
public abstract class FinanceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(FinanceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(FinanceModule).Assembly.FullName} assembly!");
        }
