...
namespace ZenCrm.Juris.Tests;

/* Inherit your integration test classes from this class */
public abstract class JurisIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(JurisModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(JurisModule).Assembly.FullName} assembly!");
        }
